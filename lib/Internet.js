    ...
    static doRequest(options, toExecute, attempts = 0) {
        ...
        var req = https.request(options, function (res) {
            if (res.statusCode === 301 || res.statusCode === 302) {
                let [host, endpoint] = Internet.extractURL(res.headers.location);
                console.log(host + ' ' + endpoint + ' ' + res.headers.location)
                options['host'] = host;
                options['endpoint'] = endpoint;
                Internet.doRequest(options, toExecute, attempts + 1);
                return;
            } else {
