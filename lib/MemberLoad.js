...
const { getProfile, setJob, addJobTime, updateBalance, checkCooldown } = require('./Member');
...
class MemberLoad {
    ...
    static async workJob(snowflake, channel, callback) {
        ...
        getProfile(snowflake, function (profile) {
            ...
            checkCooldown(snowflake, 'work', function (banned) {
                if (banned) {
                    channel.send('You must wait an hour before working again!');
                    return;
                }
                // Check level
                const [level, xp, progress] = calculateLevel(profile);
