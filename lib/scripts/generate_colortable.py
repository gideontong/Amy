...
# Adapted from Name that Color by Chirag Mehta
colors = [
["000000", "Black"],
["000080", "Navy Blue"],
["0000C8", "Dark Blue"],
...
def parseTable():
    for i in range(len(colors)):
        number = int(colors[i][0], 16)
        red, green, blue = getRGB(number)
        hue, saturation, luminance = getHSL(red, green, blue)
        colors[i] += [red, green, blue, hue, saturation, luminance]

def getName(number: int):
    color = hex(number)[2:].upper()
    red, green, blue = getRGB(number)
    hue, saturation, luminance = getHSL(red, green, blue)
    top = float('inf')
    best = ''
    for [value, name, r, g, b, h, s, l] in colors:
        if value == color:
            return name
        left = pow(red - r, 2) + pow(green - g, 2) + pow(blue - b, 2)
        right = pow(hue - h, 2) + pow(saturation - s, 2) + pow(luminance - l, 2)
        total = left + right * 2
        if top > total:
            top = total
            best = name
...
def main():
    ...
    for i in range(0xFFFFFF):
        color = hex(i)[2:].upper().zfill(6)
        # print(f'Now processing 0x{color}')
        name = getName(i)
        table[color] = name
