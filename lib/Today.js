...
class Today {
    ...
    static timeSinceDateString(date) {
        const seconds = Today.timeSinceDate(date);
        var returnString = '';
        ...
        returnString = seconds < 1 ? 'less than a second' : returnString;
        return returnString;
    }

    static timeSinceSnowflake(snowflake) {
        return Today.timeSinceDateString(Today.convertSnowflake(snowflake));
    }
}
