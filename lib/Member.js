...
class Member {
    ...
    static async setJob(snowflake, job, callback) {
        ...
        database.connect(async err => {
            ...
            const profile = await collection.findOne(filter);
            if (profile) {
                if (profile.cooldowns.job < now) {
                    await collection.updateOne(filter, update);
                    callback([true, job, tomorrow]);
                } else {
                    callback([false, 'You must wait at least 12 hours before finding a new job!']);
                }
            } else {
                await Member.createProfile(snowflake);
    ...
    static async getPremium(callback, snowflake) {
        ...
        database.connect(async err => {
            ...
            const profile = await collection.findOne(filter);
            if (profile) {
                if (now < profile.premium.expiry) {
                    callback(true, profile.premium.expiry);
                } else {
                    callback(false, now);
                }
            } else {
    ...
    static async setPremium(callback, snowflake, days = 0) {
        ...
        database.connect(async err => {
            ...
            const profile = await collection.findOne(filter);
            if (profile) {
                let expiry = profile.premium.expiry;
                expiry.setDate(expiry.getDate() + days);
                const update = {
                    $set: {
                        'profile.premium.expiry': expiry
                    }
                };
                if (now < profile.premium.expiry) {
                    await collection.updateOne(filter, update);
