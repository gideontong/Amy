...
class Survey {
    ...
    static updateExistingPoll(reaction) {
        ...
        if (pollType == 'mc') {
            ...
            var max = -1, maxIndex = -1, equality = [], total = 0;
            for (var i = 0; i < choiceCounts.length; i++) {
                total += choiceCounts[i];
                if (choiceCounts[i] == 0) {
                    total -= i;
                    break;
                }
                else if (choiceCounts[i] > max) {
                    max = choiceCounts[i];
                    maxIndex = i;
                    equality = [i];
                } else if (choiceCounts[i] == max) {
            ...
            let winningText = embed.fields[field - 1].value.split('\n')[maxIndex];
            if (max <= 1) {
                nextString = "Currently, no one has voted!";
            } else {
                let percentage = Math.round(((max - 1) / total) * 100);
                if (equality.length == 1) {
                    nextString = `**${winningText}** is in the lead with ${percentage}% of the votes.`
                } else {
                    nextString = `Multiple options are in the lead with ${percentage}% of the votes each!`
                }
            }
        } else {
