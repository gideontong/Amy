        ...
        function formatTime(time) {
            ...
            if (isNaN(time)) {
                if (time.includes(':')) {
                    ...
                    checkIterations:
                        for (var i = 0; i < clockTime.length; i++) {
                            ...
                            switch (iterations) {
                                case 1:
                                    now.setHours(clockTime[i] + timeZoneOffset);
                                    break;
                                case 2:
                    ...
                    switch (time[time.length - 1]) {
                        case 'd':
                            ...
                            break;
                        case 'h':
                            now.setHours(now.getHours() + timeInt + timeZoneOffset);
                            break;
                        case 'm':
                ...
                if (hours > now.getHours()) {
                    now.setHours(hours + timeZoneOffset); // 24 hours is auto-handled by Date()
                } else {
                    now.setHours(hours + 24 + timeZoneOffset);
                }
            }
            return "at " + (now.getHours() - 12 + timeZoneOffset) + ", in " + Today.timeSinceDateString(now.valueOf(), true);
        }
