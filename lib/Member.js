...
class Member {
    ...
    static async transferBalance(payor, payee, amount, allowNegativeBalance = false, callback = null) {
        ...
        if (!(payor && payee && amount)) {
            console.error('Tried to transfer money, missing arguments!');
            return undefinedError;
        }
        const payorFilter = {
            snowflake: payor
    ...
    /**
     * Executes callback if cooldown is not in effect
     * @param {String} snowflake Discord snowflake
     * @param {String} command Command executing
     * @param {function} callback Functino to execute
     */
    static async checkCooldown(snowflake, command, callback) {
    }
}
